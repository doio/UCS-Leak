    ...
    internal class MoveBuildingsCommand : Command
    {
        ...
        internal override void Process()
        {
            if(MovedBuilding > 0)
            {
                ...
                this.Device.Player.GameObjectManager.GetGameObjectByID(MovedBuilding).SetPositionXY(Convert.ToInt32(replacedBuildingPosition.X), Convert.ToInt32(replacedBuildingPosition.Y), this.Device.Player.Avatar.GetActiveLayout());
                this.Device.Player.GameObjectManager.GetGameObjectByID(ReplacedBuilding).SetPositionXY(Convert.ToInt32(movedBuildingPosition.X), Convert.ToInt32(movedBuildingPosition.Y), this.Device.Player.Avatar.GetActiveLayout());
            }
        }
    }
}
